...
func logf(r *http.Request, format string, args ...interface{}) {
	s, _ := r.Context().Value(ServerContextKey).(*http.Server)
...
func dirList(w http.ResponseWriter, r *http.Request, f File) {
	...
	for _, d := range dirs {
		...
		if suffix != "" && !strings.HasSuffix(name, suffix) { // patch for suffix filter, kumakichi
			continue
		}
...
func serveContent(w http.ResponseWriter, r *http.Request, name string, modtime time.Time, sizeFunc func() (int64, error), content io.ReadSeeker) {
	...
	if size >= 0 {
		...
		case len(ranges) == 1:
			...
			// a http.Request for a single range, since a client that
			// does not http.Request multiple parts might not support
			// multipart responses."
			ra := ranges[0]
...
// condResult is the result of an HTTP http.Request precondition check.
...
// checkPreconditions evaluates http.Request preconditions and reports whether a precondition
...
// actually return err.http.Error(), since msg and httpStatus are returned to users,
...
//
// As another special case, ServeFile redirects any http.Request where r.URL.Path
